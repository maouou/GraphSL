...
def get_idx_info(label, n_cls, train_mask):
    ...
    for i in range(n_cls):
        ...
        flag = flag.to(train_mask.device)
        index_list = index_list.to(train_mask.device)
        cls_indices = index_list[flag & train_mask]
...
def make_longtailed_data_remove(edge_index, label, n_data, n_cls, ratio, train_mask):
    ...
    for i in range(n_cls):
        index_list = index_list.to(original_mask.device)
...
# def make_longtailed_data_remove(edge_index, label, n_data, n_cls, ratio, train_mask):
#     '''
#      为了方便与GraphSR进行比较，添加一个函数
#      该函数作用是随机选择 Cora 和 CiteSeer 的三个类别和 PubMed 的一个类别作为少数类别。所有多数类在训练集中保留 20 个节点，少数类的数量为 20 * ratio ，其中 ratio 是不平衡比率。
#     '''
#     # Sort from major to minor
#     n_data = torch.tensor(n_data)
#     sorted_n_data, indices = torch.sort(n_data, descending=True)
...
#     for i in range(n_cls):
#         assert int(sorted_n_data[0].item() * np.power(mu, i)) >= 1
#         if i < 3:  # Top 3 classes
#             class_num_list.append(int(min(20 / ratio, sorted_n_data[i])))
#         else:  # Remaining classes
#             class_num_list.append(int(min(20, sorted_n_data[i])))
#         if i < 1: # We do not remove any nodes from the most frequent class
#             n_round.append(1)
#         else:
#             n_round.append(10)
#     class_num_list = np.array(class_num_list)
...
#     edge_mask = row_mask & col_mask
#
#     # Update train_mask and create test_mask
#     test_mask = ~train_mask
#     train_mask = node_mask & train_mask
#     test_mask = node_mask & test_mask
#
#     idx_info = []
#     for i in range(n_cls):
#         cls_indices_train = index_list[(label == i) & train_mask]
#         idx_info.append(cls_indices_train)
#
#     return list(class_num_list), train_mask, idx_info, node_mask, edge_mask
