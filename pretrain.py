...
from data_utils import get_dataset, get_idx_info, make_longtailed_data_remove
...
def train():
    ...
    pre_out = output.clone()
    loss_pre_train = criterion(output[data_train_mask], data.y[data_train_mask])
    loss_pre_train.backward()
...
def pre_train():
    ...
    for epoch in tqdm.tqdm(range(args.epoch)):
        loss, prev_out = train()
        accs, baccs, f1s, y_count, y_correct = test_pre()
